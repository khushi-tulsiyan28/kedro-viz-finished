...
""")
    ...
    if not repo_url:
        ...
        if test_result.returncode != 1:  
            ...
            raise Exception("SSH connection to GitHub failed. Please check your SSH keys.")
        
        logging.info("SSH connection test successful")

        logging.info(f"Cloning repository: {repo_url} (branch: {branch})")
        clone_cmd = ["git", "clone", "-b", branch, repo_url, str(project_dir)]
        result = subprocess.run(clone_cmd, capture_output=True, text=True, check=True, timeout=300)
...
def create_mlflow_experiment(experiment_name, pipeline_name, guid):
    try:
        mlflow_tracking_uri = f"file://{os.path.expanduser('~/mlruns')}"
...
def run_kedro_pipeline(**context):
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            env = os.environ.copy()
            env["MLFLOW_TRACKING_URI"] = f"file://{os.path.expanduser('~/mlruns')}"
